...
export const updateShopSettings = async (shop, metalRate) => {
  try {
...
export const invalidateCaches = async (shopId) => {
  try {
    await Promise.all([
...
export const createOrUpdateTodayRate = async (shopId, rateData, userId) => {
  try {
    ...
    await updateShopSettings(shop, metalRate);

    await invalidateCaches(shopId);
...
export const getCurrentRate = async (shopId) => {
  try {
...
export const getRateHistory = async (shopId, filters = {}) => {
  try {
...
export const getRateByDate = async (shopId, date) => {
  try {
...
export const compareRates = async (shopId, fromDate, toDate) => {
  try {
    ...
    const comparison = {
      ...
      gold24K: calculateRateChange(fromRate.gold.gold24K.sellingRate, toRate.gold.gold24K.sellingRate),
      gold22K: calculateRateChange(fromRate.gold.gold22K.sellingRate, toRate.gold.gold22K.sellingRate),
      gold18K: calculateRateChange(fromRate.gold.gold18K.sellingRate, toRate.gold.gold18K.sellingRate),
      silver999: calculateRateChange(fromRate.silver.pure.sellingRate, toRate.silver.pure.sellingRate),
      platinum: calculateRateChange(fromRate.platinum.sellingRate, toRate.platinum.sellingRate),
      trendComparison: {
        gold: {
...
export const getTrendChartData = async (shopId, metalType = 'gold', days = 90) => {
  try {
...
export const syncToAllShops = async (organizationId, rateData, userId) => {
  try {
    ...
    const results = await Promise.allSettled(
      shops.map(shop => createOrUpdateTodayRate(shop._id, rateData, userId))
    );
...
export const getOrganizationRate = async (organizationId) => {
  try {
...
export const deactivateRate = async (rateId, userId) => {
  try {
    ...
    await invalidateCaches(rate.shopId);
...
export const deleteRate = async (rateId, userId) => {
  try {
    const rate = await MetalRate.findById(rateId);
