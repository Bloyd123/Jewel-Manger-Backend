...
orderSchema.pre('save', function (next) {
  ...
  this.financials.estimatedTotal = this.items.reduce(
    (sum, item) => sum + item.estimatedCost.totalCost * item.quantity,
    0
  );

  // Calculate actual total
  this.financials.actualTotal = this.items.reduce(
    (sum, item) => sum + item.actualCost.totalCost * item.quantity,
    0
  );
